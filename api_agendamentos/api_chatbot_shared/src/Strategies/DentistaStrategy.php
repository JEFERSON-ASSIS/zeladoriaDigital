...
final class DentistaStrategy implements AgendamentoStrategyInterface
{
    ...
    {
        $id = (int)(AppConfig::servicos()['dentista'] ?? 0);
        return $id > 0 && $servicoId === $id;
    }

    public function agendar(array $input): array
    {
        $idServico = (int)(AppConfig::servicos()['dentista'] ?? 0);
        if ($idServico <= 0) {
            throw new \RuntimeException('Serviço dentista não configurado nesta unidade.');
        }
